...
static OfxStatus describe(OfxImageEffectHandle e){if(!gProp||!gEffect||!gParam)return kOfxStatErrMissingHostFeature;OfxPropertySetHandle p=nullptr;gEffect->getPropertySet(e,&p);S(p,kOfxPropLabel,0,"Keystone v1.6");S(p,kOfxPropShortLabel,0,"Keystone v1.6");S(p,kOfxPropLongLabel,0,"Keystone v1.6");S(p,kOfxPropVersionLabel,0,"1.6");S(p,kOfxPropPluginDescription,0,"ARRI AWG4/LogC4 grading pipeline using the original ColorGradr OFX Fix engine before Keystone grading.");S(p,kOfxImageEffectPropSupportedContexts,0,kOfxImageEffectContextFilter);S(p,kOfxImageEffectPropSupportedContexts,1,kOfxImageEffectContextGeneral);S(p,kOfxImageEffectPropSupportedPixelDepths,0,kOfxBitDepthFloat);I(p,kOfxImageEffectPropSupportsTiles,0,0);I(p,kOfxImageEffectPropSupportsMultiResolution,0,1);I(p,kOfxImageEffectPropSupportsMultipleClipDepths,0,0);I(p,kOfxImageEffectPropTemporalClipAccess,0,0);I(p,kOfxImageEffectPropRenderTwiceAlways,0,0);S(p,kOfxImageEffectPropMetalRenderSupported,0,"true");return kOfxStatOK;}
