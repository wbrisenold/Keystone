...
inline float3 applyRegionGrade(float3 lin,const Params& p,float mask){
  float m=clampf(mask*clampf(p.regionAmount,0.0f,1.0f),0.0f,1.0f);
  if(m<=1e-7f)return lin;
  float3 g=lin;
  float e=clampf(p.regionExposure,-4.0f,4.0f);
  if(_fabs(e)>1e-7f)g*=_powf(2.0f,e);
  float c=clampf(p.regionContrast,0.5f,2.0f);
  if(_fabs(c-1.0f)>1e-7f)g=keystone_skin_tone_luma(g,c,0.0f,0.0f,0.0f);
  float t=clampf(p.regionTemp,-100.0f,100.0f);
  if(_fabs(t)>1e-7f)g=ks_apply_opponent_wb_awg4(g,t,0.0f);
  float sat=clampf(p.regionSaturation,0.0f,2.0f);
  if(_fabs(sat-1.0f)>1e-7f){
    float3 rgb=ks_awg4_to_ks_skin_rec709(g);
    float3 hsv=ks_rgb_to_hsv(rgb);
    hsv.y=clampf(hsv.y*sat,0.0f,1.5f);
    g=ks_ks_skin_rec709_to_awg4(ks_hsv_to_rgb(hsv));
  }
  g=ks_soft_gamut_bend(g);
  return lin+(g-lin)*m;
}

inline float3 processPixel(float3 input,const Params& p,const LutEntry* lut,float regionMask=0.0f){
  float3 lin=applyStoredNeutral(preNeutral(input,p),p);
  if(p.regionEnable)lin=applyRegionGrade(lin,p,regionMask);
