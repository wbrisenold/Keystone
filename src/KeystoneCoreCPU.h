...
inline float3 preNeutral(float3 input,const Params& p){
  ...
  // Scene Grade color decision: exact multiplicative/additive temperature law in linear AWG4.
  float sgt=clampf(p.sceneGainTemp,-1.0f,1.0f), sot=clampf(p.sceneOffsetTemp,-1.0f,1.0f);
  if(_fabs(sgt)>1e-7f){lin.x*=(1.0f+sgt*0.20f);lin.z*=(1.0f-sgt*0.20f);}
  if(_fabs(sot)>1e-7f){lin.x+=sot*0.10f;lin.z-=sot*0.10f;}
  float sexp=clampf(p.sceneExposure,-3.0f,3.0f); if(_fabs(sexp)>1e-7f)lin*=_powf(2.0f,sexp);
  float ssh=clampf(p.sceneShadows,-1.0f,1.0f), shi=clampf(p.sceneHighlights,-1.0f,1.0f);
  if(_fabs(ssh)>1e-7f||_fabs(shi)>1e-7f)lin=primera_tone_luma(lin,1.0f,ssh,shi,0.0f);
